...
def createpost():
    ...
    if ppost.validate_on_submit(): 
        newPost = Post(title = ppost.title.data,endDate = ppost.end_date.data,qualifications = ppost.qualifications.data, description = ppost.description.data, startDate = ppost.start_date.data,commitment = ppost.commitment.data, interests = ppost.interest.data)#,faculty_id = current_user.id)
